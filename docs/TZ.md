...
| D-6 | §13 Docker | Docker в среде разработки недоступен (`docker: command not found`), файлы написаны, но `docker compose up` локально не прогонялся. Схема сети проверена заглушкой nginx на Python: продакшн-бандл, отрезание префикса `/api`, все шесть сценариев | ограничение машины; локальный запуск без Docker полностью рабочий |
