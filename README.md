...
растут линейно с числом клиентов, а память контейнера — нет. Интерфейс
хранилищ (`backend/app/store/`) для этого и отделён от сервисов —
менять придётся четыре класса, а не цепочку обработки. См. «Возможные
улучшения».
