...
плиток, названия решений и уровней риска, столбцы таблиц. Это то, чем
человек пользуется.
...
| 2 | New device | **35** | `CHALLENGE` | политика `new_device` (модель дала 17) |
| 3 | Unusual country | **55** | `CHALLENGE` | политики `high_risk_country`, `unusual_country` (модель дала 5) |
| 4 | Large amount | **57** | `CHALLENGE` | **только модель**, политики не срабатывали |
...
**Текстовый отчёт на трёх языках.** API переведён параметром
`?language=` (раздел «Три языка»), а отчёт `/report` остаётся русским:
это отдельный документ на страницу прозы, а не обвязка интерфейса.
