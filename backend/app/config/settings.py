...
class Settings(BaseSettings):
    ...
    # -------------------------------------------------------------- storage
    #: Сколько последних операций держать в истории.
    #:
    #: Десять тысяч, потому что столько требует хакатон. Прежние пять
    #: тысяч вмещали ровно половину, и проверить требование на стенде
    #: было нельзя — буфер вытеснял бы старые операции раньше, чем
    #: счётчик дошёл бы до десяти тысяч.
    #:
    #: Цена — память: запись занимает около килобайта, то есть десять
    #: мегабайт на полный буфер. Это меньше, чем весит сама модель,
    #: и на free-tier контейнере помещается.
    max_stored_transactions: int = Field(default=10_000, gt=0)
