...
class Settings(BaseSettings):
    ...
    # 220 хватало русскому и английскому, но не казахскому: специфичные
    # казахские буквы редки в словаре токенизатора, и один и тот же смысл
    # стоит там заметно дороже. На живом ответе казахский текст обрывался
    # на полуслове там, где английский укладывался с запасом.
    #
    # 400 даёт запас примерно вдвое от наблюдаемой длины. Обрыв всё равно
    # не пройдёт незамеченным: `_extract_text` смотрит `finish_reason`
    # и предпочтёт запасной текст обрубку.
    llm_max_tokens: int = Field(default=400, ge=32, le=2000)
