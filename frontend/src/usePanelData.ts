 ...
 * из `api.ts` либо обёрнутая в `useCallback`. Стрелка, созданная
 * в теле компонента, перезапускала бы запрос на каждую отрисовку.
 *
 * Зависимости этого `useCallback` — способ сказать, когда данные
 * устарели. Панели, чьи тексты приходят с backend, привязывают
 * загрузчик к языку: смена языка перезапрашивает отчёт, иначе
 * описания остались бы на прежнем.
 */
